...
import { exportDatasetUrl, exportSummaryUrl, exportLogUrl, exportAllUrl } from "../api/client";
...
export default function ExportPage() {
  ...
  const heldOutPct  = Math.round((run.held_out_size ?? 0.2) * 100);
  const trainPct    = 100 - heldOutPct;
  const heldOutRows = Math.round((run.total_before ?? 0) * (run.held_out_size ?? 0.2));
  const trainRows   = (run.total_before ?? 0) - heldOutRows;

  const logCardProps = {
    smote:    { icon: "➕", title: "Added Rows Log", desc: "CSV of all synthetic rows generated by SMOTE, with a change_type column set to \"added\"." },
    nearmiss: { icon: "➖", title: "Deleted Rows Log", desc: "CSV of all rows removed from the majority class by NearMiss, with a change_type column set to \"deleted\"." },
    combined: { icon: "🔄", title: "Changes Log",     desc: "CSV of all rows added by SMOTE and removed by NearMiss, labelled with a change_type column (\"added\" or \"deleted\")." },
  }[run.method];
    ...
    <div className="page-container">
      ...
      <div style={{ display: "grid", gridTemplateColumns: logCardProps ? "1fr 1fr 1fr" : "1fr 1fr", gap: 16, marginBottom: 16 }}>
        <ExportCard
          ...
          desc={`Training portion (${trainPct}% of original = ${trainRows.toLocaleString()} rows) after balancing with ${run.method.toUpperCase()}. The held-out ${heldOutPct}% (${heldOutRows.toLocaleString()} rows) is excluded — it was never seen by the balancing algorithm.`}
        ...
        {logCardProps && (
          <ExportCard
            icon={logCardProps.icon}
            title={logCardProps.title}
            desc={logCardProps.desc}
            url={exportLogUrl(runId)}
            filename={`changes_log_${runId.slice(0, 8)}.csv`}
          />
        )}
      </div>
      ...
      <div className="card" style={{ marginBottom: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            ...
            <div style={{ fontSize: 13, color: "var(--gray-500)" }}>All files packaged as a single ZIP archive{logCardProps ? ", including the changes log" : ""}.</div>
          </div>
          <a
